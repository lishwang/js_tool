...
- 属性描述对象提供6个元属性;
  ...
  - get  是一个函数，表示该属性的取值函数（getter），读取该属性值时会触发，默认为undefined。
  - set  是一个函数，表示该属性的存值函数（setter），修改该属性值时会触发，默认为undefined。

##### 知识补充2 -- Object.create() 创建对象
...
- ##### Object.create继承的应用
  ...
  - **相对于构造函数的继承，Object.create继承实现了将A,B的原型完美分隔 。双方不会互相影响。这是Object.create亮点所在；**
...
- 这个方法用来创建一个对象，参数是一个二维数组或者是一个Map数据

##### fromEntries 和 entries 是一对逆运算

- fromEntries 将二维数组或者Map转换成对象
- entries 将对象转换成二维数组
...
-   `flat(参数)  `    **参数表示深度，是一个数字**，如果不传参数，相当于传了1（默认值），只能将高维数组降低一维；如果想将3维数组转换为一维数组，需要传2；
...
- g 标志表示全局搜索，如果定义正则表达式时没有g修饰，则只返回首个匹配的结果。否则返回所有匹配结果。
